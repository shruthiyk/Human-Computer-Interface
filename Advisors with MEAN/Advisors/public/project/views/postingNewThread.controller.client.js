    ...
    function postingNewThreadController($routeParams, $scope, $http, UserService, ThreadService, $location) {
        ...
        function createThread(question, course, description){
            if(question != null && course != null && description != null) {
                var thread = {"query": question, "description": description, "course": course, "courseCategory": vm.courseCategory, "numberOfComments": 0, "views": 0};
            ThreadService
                    .createThread(vm.userId, thread)
                    .then(
                        function (response) {
                            var newThread = response.body;
                            $location.url("/user/" + vm.userId + "/discussionBoard");
                        },
                        function (error) {
                            vm.error = error.data;
                        }
